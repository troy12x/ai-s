...
class PaperScore:
    def __init__(self, review_data):
        ...
        def parse_score(value):
            ...
            return score_map.get(str(value).lower(), 1)
        
        # Get scores with fallbacks
        self.originality = parse_score(review_data.get('Originality', review_data.get('originality', 1)))
        self.quality = parse_score(review_data.get('Quality', review_data.get('quality', 1)))
        self.clarity = parse_score(review_data.get('Clarity', review_data.get('clarity', 1)))
        self.significance = parse_score(review_data.get('Significance', review_data.get('significance', 1)))
        
        # Handle different overall score formats
        overall = review_data.get('Overall', review_data.get('overall', None))
        if overall is None:
            # Calculate from component scores if not provided
            self.overall = self.get_average_score() * 2.5  # Scale to 1-10
        else:
            self.overall = float(overall)
    
    def get_average_score(self):
        return (self.originality + self.quality + self.clarity + self.significance) / 4
    
    def needs_improvement(self, target_score=6.0):
        return self.overall < target_score

def analyze_review_feedback(review_data):
    """Analyze review feedback to identify specific areas needing improvement."""
    improvements = {
        'methodology': [],
        'experiments': [],
        'presentation': [],
    ...
    # Extract improvement suggestions from weaknesses
    for weakness in review_data.get('Weaknesses', []):
        if any(term in weakness.lower() for term in ['method', 'approach', 'algorithm']):
            improvements['methodology'].append(weakness)
        elif any(term in weakness.lower() for term in ['experiment', 'result', 'evaluation']):
            improvements['experiments'].append(weakness)
        elif any(term in weakness.lower() for term in ['writing', 'clarity', 'structure']):
            improvements['presentation'].append(weakness)
        elif any(term in weakness.lower() for term in ['theory', 'proof', 'theoretical']):
            improvements['theory'].append(weakness)
    
    return improvements

def generate_improvement_plan(improvements, review_data):
    """Generate specific steps to improve the paper based on review feedback."""
    score = PaperScore(review_data)
    ...
    if score.quality < 3:
        plan['high_priority'].extend(improvements['methodology'])
        plan['high_priority'].extend(improvements['experiments'])
    if score.clarity < 3:
        plan['high_priority'].extend(improvements['presentation'])
    if score.originality < 3:
        plan['medium_priority'].extend(improvements['theory'])
    
    return plan

def apply_improvements(coder, folder_name, improvement_plan):
    """Apply improvements to the paper based on the improvement plan."""
    latex_file = osp.join(folder_name, 'latex', 'template.tex')
    
    # Apply high priority improvements first
    for improvement in improvement_plan['high_priority']:
        prompt = f"""
        Based on this review feedback: "{improvement}"
        Please improve the relevant section in the paper. Focus on:
        1. Adding missing content
        2. Clarifying methodology
        3. Strengthening arguments
        4. Adding necessary citations
        
        Use SEARCH/REPLACE to make the changes in {latex_file}.
        """
        coder.run(prompt)
    
    # Apply medium priority improvements
    for improvement in improvement_plan['medium_priority']:
        prompt = f"""
        Based on this review feedback: "{improvement}"
        Make targeted improvements to address this point.
        
        Use SEARCH/REPLACE to make the changes in {latex_file}.
        """
        coder.run(prompt)

def iterative_improvement(text, model, client, coder, folder_name, target_score=6.0, max_iterations=3):
    """Iteratively improve paper quality using review feedback."""
    review_history = []
    
    # Handle both file paths and text content
    if isinstance(text, str) and os.path.exists(text):
        try:
            text = load_paper(text)
        except Exception as e:
    ...
    for iteration in range(max_iterations):
        try:
            ...
            score = PaperScore(review)
            if not score.needs_improvement(target_score):
                print(f"Target score reached: {score.overall}/10")
                break
            
            # Enhanced analysis
            improvements = analyze_review_feedback(review)
            citation_issues = enhance_citations(review, text)
            writing_issues = check_writing_consistency(text)
            
            # Generate comprehensive improvement plan
            plan = generate_improvement_plan(improvements, review)
            
            # Add citation and writing improvements
            if citation_issues:
                plan['high_priority'].extend([f"Fix citation issue: {issue['issue']}" for issue in citation_issues])
            if writing_issues['structure']:
                plan['medium_priority'].extend(writing_issues['structure'])
            
            # Apply improvements
            apply_improvements(coder, folder_name, plan)
            
            # Recompile paper and update text
            try:
                from ai_scientist.perform_writeup import generate_latex
                generate_latex(coder, folder_name, "paper.pdf")
                text = load_paper(os.path.join(folder_name, "paper.pdf"))
            except Exception as e:
                print(f"Error recompiling paper: {e}")
                break
            
            # Track improvement history
            if len(review_history) > 1:
                changes = track_improvement_history(review_history)
                print(f"\nImprovements in iteration {iteration + 1}:")
                for metric, delta in changes['score_changes'][-1].items():
                    if delta > 0:
                        print(f"- {metric}: +{delta:.2f}")
            
            print(f"Iteration {iteration + 1}: Current score = {score.overall}/10")
        except Exception as e:
            print(f"Error in improvement iteration {iteration + 1}: {e}")
            break
    
    return review, review_history

def track_improvement_history(review_history):
    """Track how specific aspects improve over iterations."""
    changes = {
        'methodology_changes': [],
        'experiment_changes': [],
    ...
    for i in range(1, len(review_history)):
        prev = review_history[i-1]
        curr = review_history[i]
        
        # Track score changes
        prev_score = PaperScore(prev)
        curr_score = PaperScore(curr)
        changes['score_changes'].append({
            'iteration': i,
            'originality_delta': curr_score.originality - prev_score.originality,
            'quality_delta': curr_score.quality - prev_score.quality,
            'clarity_delta': curr_score.clarity - prev_score.clarity,
            'significance_delta': curr_score.significance - prev_score.significance
        })
    
    return changes

def enhance_citations(review_data, paper_text):
    """Identify and suggest missing citations based on review feedback."""
    citation_issues = []
    
    # Check review feedback for citation issues
    for weakness in review_data.get('Weaknesses', []):
        if any(term in weakness.lower() for term in ['citation', 'reference', 'prior work']):
            citation_issues.append({
                'issue': weakness,
                'type': 'missing_citation' if 'missing' in weakness.lower() else 'inadequate_citation'
            })
    
    # Check citation patterns in text
    citation_patterns = re.findall(r'\\cite{([^}]*)}', paper_text)
    if len(citation_patterns) < 10:  # Arbitrary threshold
        citation_issues.append({
            'issue': 'Low overall citation count',
            'type': 'insufficient_citations'
        })
    
    return citation_issues

def check_writing_consistency(paper_text):
    """Check for consistent writing style and terminology."""
    issues = {
        'terminology': [],
        'formatting': [],
        'structure': []
    ...
    for section in required_sections:
        if not re.search(rf'\\section{{.*{section}.*}}', paper_text, re.IGNORECASE):
            issues['structure'].append(f'Missing {section} section')
    
    # Check formatting consistency
    if len(re.findall(r'\\paragraph{', paper_text)) < 3:
        issues['formatting'].append('Consider adding more paragraph breaks')
