...
from ai_scientist.perform_review import perform_review, load_paper, perform_improvement, iterative_improvement
...
def do_idea(
    ...
    """Execute a research idea with iterative improvement."""
    print("Starting idea:", idea["Name"])
    ...
    try:
        ...
        if writeup == "latex" and improvement:
            ...
            try:
                review = iterative_improvement(
                    text=f"{folder_name}/{idea['Name']}.pdf",
                    model=model,
                    client=client,
                    coder=coder,
                    folder_name=folder_name,
                    target_score=6.0,  # Target high-scoring papers
                    max_iterations=3
                )
                # Save final review
                review_file = osp.join(folder_name, "final_review.json")
